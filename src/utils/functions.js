...
module.exports = (client) => {
	const { UserModel, PresentationModel, mongoose } = client;
  ...
  client.getAgeFromDate = (date) => {
	...
	client.setPresentation = async ({guild, member}, presentation) => {
		let { user } = member;
	...
	client.getPresentationDb = async ({guild, member}) => {
		let { user } = member;
	...
	client.getPresentation = async ({guild, member}, statusMatter) => {
		let { user } = member;
		...
		if(presentationDb) {
			return await client.presentationFieldsParse({guild, member}, presentationDb);
		}
		return `❌ La présentation de ce membre n'existe pas ou n'est pas publiée.`;
	}


	client.presentationFieldsParse = ({member, guild}, values) => {
		const fieldsName = {
